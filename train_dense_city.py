...
if __name__ == "__main__":
    ...
    # ----------------------------------------------------------------------------------------------------------------------------#
    model_path = "model_data/Dense_miou_67.39.pth"
    # ---------------------------------------------------------#
    ...
    # ------------------------------------------------------#
    #   设置用到的显卡
    #   加载backbone:“model_path”用到的显卡: device
    # ------------------------------------------------------#
    ...
    model = DeepLab_Dense(num_classes=num_classes, backbone=backbone, downsample_factor=downsample_factor,
    ...
    if local_rank == 0:
        ...
        log_dir = os.path.join(save_dir, "City_loss_" + str(time_str))
    ...
    if local_rank == 0:
        ...
        # ---------------------------------------------------------#
        #   总训练世代（epoch）指的是遍历全部数据的总次数
        #   总训练步长(step)指的是梯度下降的总次数
        ...
        if total_step <= wanted_step:
            ...
            print("\n\033[1;33;40m[Warning] 使用%s优化器时，建议将训练总步长设置到%d以上。\033[0m" % (optimizer_type, wanted_step))
            print("\033[1;33;40m[Warning] 本次运行的总训练数据量为%d，Unfreeze_batch_size为%d，共训练%d个Epoch，计算出总训练步长为%d。\033[0m" % (
            num_train, Unfreeze_batch_size, UnFreeze_Epoch, total_step))
            print("\033[1;33;40m[Warning] 由于总训练步长为%d，小于建议总步长%d，建议设置总世代为%d。\033[0m" % (
    ...
    if True:
        ...
        train_dataset = DeeplabDataset(train_lines, input_shape, num_classes, train=True, dataset_path=VOCdevkit_path)
        val_dataset = DeeplabDataset(val_lines, input_shape, num_classes, train=False, dataset_path=VOCdevkit_path)
        ...
        # ---------------------------------------------------#
        #  是否将val纳入train dataset
        # ---------------------------------------------------#
        Cat_val2train_flag = True
        if Cat_val2train_flag:
            epoch_step = (num_train + num_val) // batch_size
            concat_dataset = ConcatDataset([train_dataset, val_dataset])
            gen = DataLoader(concat_dataset, shuffle=shuffle, batch_size=batch_size, num_workers=num_workers,
                             pin_memory=True,
